...
def assemble(lines: list[str]) -> None:
    ...
    for line_num, x in enumerate(lines):
        ...
        inst_type, func = opcode_table[inst]

        if inst_type == 'RR':
            check_num_args(parts, 2, line_num)
            check_registers_exist(parts[1:], line_num)
            as_print(func(reg_codes[parts[1]], reg_codes[parts[2]])) # type: ignore
        elif inst_type == 'R8U':
            check_num_args(parts, 2, line_num)
            imm, r2 = parts
            if imm[0] != '$':
            ...
            check_unsigned_range(parsed_imm, 8, line_num)
            check_registers_exist([r2], line_num)
            as_print(func(parsed_imm, reg_codes[r2])) # type: ignore
            ...
            if imm[0] != '$':
                ...
                sys.exit(f'Error: symbol table not implemented: bad imm {imm} on line {line_num}')
            parsed_imm = int(imm[1:])
            check_signed_range(parsed_imm, 8, line_num)
            check_registers_exist([r2], line_num)
            as_print(func(parsed_imm, reg_codes[r2])) # type: ignore
            ...
            if imm[0] != '$':
                ...
                sys.exit(f'Error: symbol table not implemented: bad imm {imm} on line {line_num}')
            parsed_imm = int(imm[1:])
            check_signed_range(parsed_imm, 5, line_num)
            check_registers_exist([r2], line_num)
            as_print(func(parsed_imm, reg_codes[r2])) # type: ignore
            ...
            if imm[0] != '$':
                ...
                sys.exit(f'Error: symbol table not implemented: bad imm {imm} on line {line_num}')
            parsed_imm = int(imm[1:])
            check_unsigned_range(parsed_imm, 4, line_num)
            check_registers_exist([r2], line_num)
            as_print(func(parsed_imm, reg_codes[r2])) # type: ignore
            
        elif inst_type == '4U':
            check_num_args(parts, 1, line_num)
            imm = parts[0]
            if imm[0] != '$':
                # TODO
                sys.exit(f'Error: symbol table not implemented: bad imm {imm} on line {line_num}')
            parsed_imm = int(imm[1:])
            check_unsigned_range(parsed_imm, 4, line_num)
            as_print(func(parsed_imm)) # type: ignore
            
        elif inst_type == 'N':
            check_num_args(parts, 0, line_num)
            as_print(func()) # type: ignore
