...
class Assembler():
    labels = {}
    jpoint_instrs = {}
    RType = ['ADD', 'ADDU', 'ADDC', 'ADDCU', 'SUB','SUBC', 'CMP', 'CMPU', 'AND', 'OR', 'XOR', 'MUL']
    Immediates = ['ADDI', 'ADDUI', 'ADDCI', 'SUBI', 'SUBCI','CMPI', 'CMPUI', 'ANDI', 'ORI', 'XORI','MULI']
    ...
    def instrCode(self, name) -> str:
        return self.switcher[name]
                ...
                if (instr in self.RType):
                    if (len(parts) == 2):
                        ...
                        if ((firstReg in self.REGISTERS) and (secondReg in self.REGISTERS)):
                            ...
                            data = '0' + secondRegNum + self.instrCode(instr) + firstRegNum
                            wf.write(data + '\n')
                        else:
                   ...
                   if (len(parts) == 2):
                        ...
                        if ((firstReg in self.REGISTERS) and (secondReg in self.REGISTERS)):
                            firstRegNum = '{0:01x}'.format(int(firstReg.replace('%r', '')))
                            secondRegNum = '{0:01x}'.format(int(secondReg.replace('%r', '')))
                            data = '8' + secondRegNum + self.instrCode(instr) + firstRegNum
                            wf.write(data + '\n')
                        else:
                    ...
                    if (len(parts) == 2):
                        ...
                        if ((Immd[0] in '$') and (secondReg in self.REGISTERS)):
                            ...
                            secondRegNum = '{0:01x}'.format(int(secondReg.replace('%r', '')))
                            data = '8' + secondRegNum + self.instrCode(instr) + immediate
                            wf.write(data + '\n')
                        else:
                    ...
                    if (len(parts) == 1):
                        ...
                        if (Disp[0] in '$'):
                            ...
                            data = 'b' + self.instrCode(instr.replace('B', '')) + Displacement
                            wf.write(data + '\n')
                        elif (Disp[0] == '.'):
                    ...
                    if (len(parts) == 1):
                        ...
                        if ((firstReg in self.REGISTERS)):
                            firstRegNum = '{0:01x}'.format(int(firstReg.replace('%r', '')))
                            data = '4' + self.instrCode(instr.replace('J', '')) + 'c' + firstRegNum
                            wf.write(data + '\n')
                        else:
